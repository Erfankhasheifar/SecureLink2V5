 ...
 *
 * heap_4.c – First-fit allocator with block merging.
 */
...
/* Marks a block as being allocated (MSB of xBlockSize) */
...
/* The heap is statically allocated */
...
/* Keeps track of the number of calls to allocate and free memory as well
 * as the number of free bytes remaining, but says nothing about
 * fragmentation. */
...
static void prvInsertBlockIntoFreeList( BlockLink_t * pxBlockToInsert )
{
    ...
    /* Iterate through the list until a block is found that has a higher
     * address than the block being inserted. */
    for( pxIterator = &xStart;
         pxIterator->pxNextFreeBlock < pxBlockToInsert;
         pxIterator = pxIterator->pxNextFreeBlock )
    {
        /* Nothing to do here, just iterating to the correct position. */
    }
    ...
    /* If the block being inserted plugged a gap, so we were not merging with the
     * block in front, the block in front must be updated to point back to us. */
...
void * pvPortMalloc( size_t xWantedSize )
{
    ...
    vTaskSuspendAll();
    {
        ...
        /* If the end marker was reached then a block of adequate size was not
         * found. */
        if( pxBlock != pxEnd )
        {
            ...
            /* The block is being returned – it is allocated and owned by the
             * application and has no "next" block. */
